...
public class PageNavigator<T> {
    Page<T> pageFromJPA; //jpa 传递出来的分页对象， PageNavigator 类就是对它进行封装以达到扩展的效果
    ...
    public PageNavigator(){
        //这个空的分页是为了 Redis 从 json格式转换为 PageNavigator 对象而专门提供的
    }
    ...
    private void calcNavigatepageNums(){
        ...
        int num = getNumber();
        //当总页数小于或等于导航页码数时
        if (totalPages <= navigatePages) {// 4 6
            navigatepageNums = new int[totalPages];//4
